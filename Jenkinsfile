...
pipeline {
    ...
    stage('Deploy to dev namespace') {
      steps {
        container('helm') {
          ...
          sh "cd keptn-config && helm upgrade --install ${env.GITHUB_ORGANIZATION}-dev ./helm-chart --namespace dev"
        }
      }
    }  
  ...
  post {
    always {
      container("curl") {
        sh "curl -X POST \"http://35.239.153.189:8080/jenkinsOperator\" -H \"accept: application/json\" -H \"Content-Type: application/json\" -d \"{ \\\"status\\\": \\\"success\\\", \\\"environment\\\": \\\"dev\\\", \\\"service\\\": \\\"${env.APP_NAME}\\\", \\\"github_org\\\": \\\"${env.GITHUB_ORGANIZATION}\\\", \\\"pull_request\\\": \\\"${PULL_REQUEST}\\\"}\" "       
      }
    }
  }
}
